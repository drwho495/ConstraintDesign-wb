...
from Utils import isType, getDistanceToEntity, generateHashName, makeBoundaryCompound, dressupTypes
...
class Pattern(Feature):
    ...
    def updateProps(self, obj, patternType=0):
        super(Pattern, self).updateProps(obj)
    ...
    def generateShape(self, obj, prevShape):
        ...
        constantXAxisVector = App.Vector(obj.XAxisLength, 0, 0)
        xAxisVector = App.Vector(obj.XAxisLength, 0, 0)
        ...
        for feature in obj.Features:
            ...
            for _, v in featureShapes.items():
                ...
                if patternShape.isNull():
                    if not remove:
                        patternShape = featureShape
                    print("set shape")
                else:
                    if remove:
                        patternShape = patternShape.cut(featureShape)
                        print("cut")
                    else:
                        patternShape = patternShape.fuse(featureShape)
                        print("fuse")
        ...
        if obj.PatternType == 0:
            for i in range(obj.XAxisCount - 1):
                newBoundary = baseBoundarySh.copy()
                newBoundary.Placement.Base += xAxisVector
                boundaryCompound = Part.Compound([boundaryCompound, newBoundary])

                newPatternShape = patternShape.copy()
                newPatternShape.Placement.Base += xAxisVector
                newShape = Part.Compound([newShape, newPatternShape])
                ...
                xAxisVector += constantXAxisVector
                occurence += 1
